...
public class TrackerHSB1 {
	...
	public double peakThresholdPercent;
	public int previouArea;
	public boolean firstTime = true;
	public int hue;
	public int saturation;
	public double centroidThreshold;
	public double centroidThresholdPercent;
	public int incr;
	BufferedImage image;
	...
	public double sakelValue;
	public int tempPeakValue;
	public int hueGreater;
	public double divideByConst;
	public double limit;
	public int mainChannel;
	...
	public TrackerHSB1() {
		...
		/*peakThreshold = 2000;
		centroidThreshold =5000;*/
		imageW = 640;
		imageH = 480;

		maskW = 40;
		maskH = 40;
		divideByConst=maskW*maskH;
		System.out.println("W" + maskW + "h" + maskH);
		previousP = new Point();
		previousP.setX(-1);

	}
	public void setMainChannel(int c){
		mainChannel=c;
	}
	public void setPeakThresholdPercent(double peakThresholdPercent) {
		this.peakThresholdPercent=peakThresholdPercent;
		
	}
	public void setCentroidThresholdPercent(double centroidThresholdPercent) {
		this.centroidThresholdPercent=centroidThresholdPercent;
	...
	public void setImage(BufferedImage im) {
		...
		image = im;
		setim++;
		System.out.println("setim:  "+setim);
		imageH = image.getHeight(null);
		imageW = image.getWidth(null);
		matchedValue = new double[imageH][imageW];

		check = new boolean[imageH][imageW];
		imageArray = new double[imageH][imageW];
	...
	public void writeOnArray() {
		...
		for (y = 0; y < imageH; y++) {
			for (x = 0; x < imageW; x++) {
				...
				if (hsb[0] > hueGreater)
					hsb[0] = 360 - hsb[0];
				imageArray[y][x] =  (hsb[0]-hue)*(hsb[0]-hue)*(hsb[1]-saturation)*(hsb[1]-saturation);
				check[y][x]=true;
			}
		}
	...
	public Point trackMarker() {
		...
		for (i = maskH / 2; i < imageH - maskH / 2 ; i += incr) {
			...
			for (j = maskW / 2 + incr; j < imageW - maskW / 2 ; j += incr) {
				...
				//if(j==605|j==610|j==615)System.out.println("i="+i+"j="+j+"m="+matchedValue[i][j]);
		...
		sakelValue = peak;
		peakThreshold= (peak*(peakThresholdPercent/100));
		centroidThreshold=(peak*(centroidThresholdPercent/100));
		System.out.println(peakThresholdPercent+"lsdf"+centroidThresholdPercent);
		System.out.println(peakThreshold+"  ld"+centroidThreshold);
		
		
		System.out.print("matched=" + peak + " x= " + p.x + " y= " + p.y + "here  ");
		/*if(peak>limit){
			p.setX(-1);
			return p;
		}*/
	...
	public Point sayemSearch(Point previous) {
		...
		double peak = 999999999.00;
		int value = 0, leftX, leftY, rightX, rightY, neighbour = 0;
		Point p = new Point();
		...
		for (neighbour = 0;; neighbour += incr) {
			...
			if (neighbour == 0) {
				...
				if (Math.abs(sakelValue
					...
					p = this.getCentroid(previous, matchedValue[previous.getY()][previous.getX()]);
					 System.out.println("1st y="+p.y+"x="+p.x);
					/*if (p.getX() == -1)
						return p;
					this.changeTemplate(p);*/
			...
			if (k >= maskH / 2) {
				...
				if (Math.abs(matchedValue[leftY][leftX] - sakelValue) < peakThreshold) {
					...
					p = this.getCentroid(p, matchedValue[leftY][leftX]);
					/*if (p.getX() == -1)
						return p;*/
					//this.changeTemplate(p);
					
					  System.out.println("second return upper value=+" +
				...
				for (j = leftX + incr; j <= rightX; j += incr) {
					...
					if (Math.abs(sakelValue - matchedValue[leftY][j]) < peakThreshold) {
						...
						p = this.getCentroid(p,  matchedValue[leftY][j]);
						// System.out.println("1st y="+p.y+"x="+p.x);
						/*if (p.getX() == -1)
							return p;
						this.changeTemplate(p);*/
					...
					if (peak > matchedValue[leftY][j] ) {
						System.out.println("p1 " + matchedValue[leftY][j]);
			...
			if (k >= maskW / 2) {
				...
				for (i = leftY + incr; i <= rightY; i += incr) {
					...
					if (Math.abs(sakelValue - matchedValue[i][leftX]) < peakThreshold) {
						...
						p = this.getCentroid(p,  matchedValue[i][leftX]);
					...
					if (peak > matchedValue[i][leftX]) {
						System.out.println("p2 " + matchedValue[i][leftX]);
			...
			if (k <= imageW - maskW / 2 - 1) {
				...
				if (Math.abs(matchedValue[leftY][rightX] - sakelValue) < peakThreshold) {
					p.setY(leftY);
					p.setX(rightX);

					p = this.getCentroid(p, matchedValue[leftY][rightX]);
					/*if (p.getX() == -1)
						return p;
					this.changeTemplate(p);*/
					
					  System.out.println("second return upper value=+" +
					  matchedValue[leftY][rightX]);
					 
					return p;
				}
				if (matchedValue[leftY][rightX] < peak ) {
					peak = matchedValue[leftY][rightX];
					p.setX(rightX);
					p.setY(leftY);
				...
				for (i = leftY + incr; i <= rightY; i += incr) {
					...
					matchedValue[i][rightX] = downPointMatchedValue(i, rightX,incr);
					...
					if (Math.abs(sakelValue - matchedValue[i][rightX]) < peakThreshold) {
						...
						p = this.getCentroid(p,  matchedValue[i][rightX]);
						/*if (p.getX() == -1)
							return p;
						this.changeTemplate(p);*/
					...
					if (peak > matchedValue[i][rightX]) {
						System.out.println("p3 " + matchedValue[i][rightX]);
			...
			if (k <= imageH - maskH / 2 - 1) {
				if(matchedValue[rightY][leftX]==0){
				  matchedValue[rightY][leftX]=curPointMatchedValue(rightY, leftX);
				}
				if (Math.abs(matchedValue[rightY][leftX] - sakelValue) < peakThreshold) {
					p.setY(rightY);
					p.setX(leftX);

					p = this.getCentroid(p, matchedValue[rightY][leftX]);
					/*if (p.getX() == -1)
						return p;
					this.changeTemplate(p);*/
					
					  System.out.println("second return upper value=+" +
					  matchedValue[rightY][leftX]);
					 
					return p;
				}
				if (matchedValue[rightY][leftX] < peak ) {
					peak = matchedValue[rightY][leftX];
					p.setX(leftX);
					p.setY(rightY);
				...
				for (j = leftX + incr; j < rightX - incr; j += incr) {
					...
					if (Math.abs(sakelValue - matchedValue[rightY][j]) < peakThreshold) {
						...
						p = this.getCentroid(p,  matchedValue[rightY][j]);
						/*if (p.getX() == -1)
							return p;
						this.changeTemplate(p);*/
					...
					if (peak > matchedValue[rightY][j]) {
						System.out.println("p4 " + matchedValue[rightY][j]);
		...
		for (k = -maskH / 2; k <= maskH / 2; k++) {
			for (l = -maskW / 2; l <= maskW / 2; l++) {
				if (check[k + y][x + l] == false) {
					...
					h = (int) (hsb[0]);
					s = (int) (hsb[1]);
					imageArray[k + y][x + l]=(h-hue)*(h-hue)+(s-saturation)*(s-saturation);
					check[k + y][x + l] = true;
				} 
				tHue += h;
				tSaturation += s;
	...
	public Point getCentroid(Point p, double peak) {
		//if(peak!=0)return p;*/
		/*Point p=new Point();
		p.setX(pa.getX());p.setY(pa.getY());*/
		if(peak>limit){
			p.setX(-1);
			return p;
		}
		...
		while (list.isEmpty() == false) {
			...
			for (i = 0; i < 8; i++) {
				 ...
				 * matchedValue[tempY][tempX]);
				 */
				if (Math.abs(matchedValue[tempY][tempX] - peak) <= centroidThreshold) {
					/*
			...
			if(count>5000){
				p.x=-1;
				System.out.println("large frm centroid 1");
				return p;
			}
		...
		/*if (Math.abs(previouArea - count) > 300 && firstTime == false) {
			p = trackMarker();
			//p = getCentroid(p, peak);
			System.out.println("returned frm centroid 2");
			p.setX(-1);
			return p;
		}
*/
		if (count > 20) {
			...
			System.out.println("small frm centroid 3");
			p.setX(-1);
			//return p;
		}
	...
	public double rightPointMatchedValue(int i, int j, int localIncr) {
		j = j - localIncr;
		double value = 0, subtract = 0, add = 0, h = 0, s = 0;
		int x, y;
		for (y = -maskH / 2; y <= maskH / 2; y++) {
			for (x = -maskW / 2; x < -maskW / 2 + localIncr; x++) {
				// System.out.print("x="+(x+j)+"y="+(y+i));
				
					subtract+=imageArray[i + y][j + x];
		...
		for (y = -maskH / 2; y <= maskH / 2; y++) {
			for (x = maskW / 2 + 1; x <= maskW / 2 + localIncr; x++) {
				if (check[i + y][j + x] == false) {
					...
					h = (int) (hsb[0]);
					s = (int) (hsb[1]);
					//imageArray[i + y][j + x] =  (h-hue)*(h-hue)+();
					imageArray[i + y][j + x] =  (h-hue)*(h-hue)+(s-saturation)*(s-saturation);
					
					check[i + y][j + x] = true;
				} 
					add+=imageArray[i + y][j + x];
				
			}
		}
		double temp= (add - subtract)/divideByConst;
		return (matchedValue[i][j] + temp);
	}

	public double leftPointMatchedValue(int i, int j, int localIncr) {
		j = j + localIncr;
		double value = 0, subtract = 0, add = 0;
		double s = 0;
		double h = 0;
		int x, y;
		for (y = -maskH / 2; y <= maskH / 2; y++) {
	...
	public double downPointMatchedValue(int i, int j, int localIncr) {
	...
	public double topPointMatchedValue(int i, int j, int localIncr) {
		i = i + localIncr;
		double value = 0, subtract = 0, add = 0;
		double s = 0;
		double h = 0;
	...
	public double curPointMatchedValue(int i, int j) {
		double value = 0;
		double s = 0;
		double h = 0;
		int x, y;
		for (y = -maskH / 2; y <= maskH / 2; y++) {
			for (x = -maskW / 2; x <= maskW / 2; x++) {
				if (check[i + y][j + x] == false) {
					...
					value+=imageArray[i + y][j + x];
				
			}
		}
		return (value/divideByConst);
	}
	public void test(){
		int x=605,y=270;
		
		matchedValue[y][x]=curPointMatchedValue(y,x);
		matchedValue[y-incr][x]=topPointMatchedValue(y-incr,x, incr);
		matchedValue[y+incr][x]=downPointMatchedValue(y+incr,x, incr);
		matchedValue[y][x-incr]=leftPointMatchedValue(y,x-incr, incr);
		matchedValue[y][x+incr]=rightPointMatchedValue(y,x+incr, incr);
		System.out.println("cur "+matchedValue[y][x]);
		System.out.println(matchedValue[y-incr][x]+"t  ="+curPointMatchedValue(y-incr,x));
		System.out.println(matchedValue[y+incr][x]+"d  ="+curPointMatchedValue(y+incr,x));
		System.out.println(matchedValue[y][x-incr]+"l  ="+curPointMatchedValue(y,x-incr));
		System.out.println(matchedValue[y][x+incr]+"r  ="+curPointMatchedValue(y,x+incr));
	}
	public void saveImage(){
		BufferedImage im =image ;
		int i=0;
		try {
		ImageIO.write((RenderedImage) im, "jpg", new File("E:" + "\\save"+ "\\" +"img"+i+ ".jpg"));
		i++;
