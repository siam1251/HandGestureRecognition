...
public class TrackerHSB2 {
	...
	public TrackerHSB2() {
		...
		try {
			maskIm = ImageIO.read(new File("E:\\save\\marker2.jpg"));
		} catch (IOException e) {
		...
		for (y = 0; y < maskH; y++) {
			for (x = 0; x < maskW; x++) {
				...
				hsb[0]=hsb[0]*360;
				hsb[1]=hsb[1]*100;
				if(hsb[0]>180)hsb[0]=360-hsb[0];
				mask[y][x][0] =(int) (hsb[0]);
				mask[y][x][1] = (int) (hsb[1]);
	...
	public Point trackMarker() {
		...
		sakelValue = peak;
		System.out.print("matched="+peak + " x= " + p.x + " y= " + p.y + "  ");

		return p;
	}
	
	public Point trackMarker2(){
		Point p = new Point();
		int i, j = 0, k, l;
		int peak = 0, value = 0;
		for (i = maskH / 2; i < imageH - maskH / 2; i++) {
			for (j = maskW / 2; j < imageW - maskW / 2; j++) {
				...
				for (k = -maskH / 2; k < maskH / 2; k++) {
					for (l = -maskW / 2; l < maskW / 2; l++) {
						...
						if (mask[maskH / 2 + k][maskW / 2 + l][0]
								 <= 100 && mask[maskH / 2 + k][maskW / 2 + l][0]>=90
								&& mask[maskH / 2 + k][maskW / 2
										+ l][1] <= 30 && mask[maskH / 2 + k][maskW / 2 +l][1] >=20) {
							value++;
						}
						// System.out.println(value);
					}
	...
	public Point sayemSearch(Point previous) {
		...
		try {
			file=new FileWriter("output1.txt");
		} catch (IOException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		}
		...
		for (neighbour = 0;; neighbour += 2) {
			...
			if(leftX<maskW/2&&leftY<maskH/2&&rightY>(imageH-maskH/2)&&rightX>(imageW-maskW/2)){
			...
			if(leftX<maskW/2)leftX=maskW/2;
			if(rightX>imageW-maskW/2)rightX=imageW-maskW/2;
			if(leftY<maskH/2)leftY=maskH/2;
			if(rightY>imageH-maskH/2)rightY=imageH-maskH/2;
			...
			if (k > maskH / 2 ) {
				...
				for (j = leftX; j <= rightX; j++) {
					...
					if(Math.abs(sakelValue - value) <= differenceValue){p.setY(leftY);p.setX(j);System.out.println("p "+peak);return p;}
					value = 0;
			...
			if ( k < imageH - maskH / 2) {
				...
				for (j = leftX; j <= rightX ; j++) {
					...
					if(Math.abs(sakelValue - peak) <= differenceValue){p.setX(j);p.setY(rightY);System.out.println("p "+peak);return p;}
					value = 0;
			...
			if(k>maskW/2){
				...
				for(i=leftY+1;i<rightY-1;i++){
					...
					if(Math.abs(sakelValue - peak) <= differenceValue){p.setX(leftX);p.setY(i);System.out.println("p "+peak);return p;}
					value = 0;
				}
			}
			// j is x axis and k is y axis
			...
			// right column scan starts
						...
						if(k<imageW-maskW/2){
							...
							for(i=leftY+1;i<rightY-1;i++){
								...
								if(Math.abs(sakelValue - peak) <= differenceValue){p.setX(rightX);p.setY(i);System.out.println("p "+peak);return p;}
						...
						if (Math.abs(sakelValue - peak) <= differenceValue) {
		...
		for (neighbour = 0;; neighbour += 1) {
			...
			if (k > maskH / 2 ) {
				...
				for (j = leftX; j <= rightX; j++) {
					...
					if(Math.abs(sakelValue - value) <= 10){p.setY(leftY);p.setX(j);System.out.println("p "+peak);return p;}
					value = 0;
			...
			if ( k < imageH - maskH / 2) {
				...
				for (j = leftX; j <= rightX ; j++) {
					...
					if(Math.abs(sakelValue - peak) <= 10){p.setX(j);p.setY(rightY);System.out.println("p "+peak);return p;}
					value = 0;
			...
			if(k>maskW/2){
				...
				for(i=leftY+1;i<rightY-1;i++){
					...
					if(Math.abs(sakelValue - peak) <= 10){p.setX(leftX);p.setY(i);System.out.println("p "+peak);return p;}
					value = 0;
				}
			}
			// j is x axis and k is y axis
			...
			// right column scan starts
						...
						if(k<imageW-maskW/2){
							...
							for(i=leftY+1;i<rightY-1;i++){
								...
								if(Math.abs(sakelValue - peak) <= 10){p.setX(rightX);p.setY(i);System.out.println("p "+peak);return p;}
